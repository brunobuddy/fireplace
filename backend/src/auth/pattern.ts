/**
 * Unlock-pattern maths, duplicated verbatim in the SPA
 * (`frontend/src/features/auth/pattern.ts`). Both copies are pinned to
 ...
 * pattern *is* the password: it is bcrypt-hashed into `AUTH_USERS` like any
 * other secret, and `AuthService` never learns it came from a grid.
 */
 ...
 * visited. Cheap enough to run before bcrypt, which is the point — it rejects
 * garbage without spending ~100ms hashing it.
 */
